...
namespace Coders.Data_Layer
{
    public class DLLandmarks
    {
        ...
        public List<NewLandmarkView> ReturnNewLandmarks()
        {
            using (db = new CodersEntities())
            {
                List<NewLandmark> ladmarks = (from l in db.NewLandmarks select l).ToList();
                List<NewLandmarkView> landmarksView = new List<NewLandmarkView>();
                foreach (NewLandmark l in ladmarks)
                {
                    landmarksView.Add(new NewLandmarkView(l));
                }
                return landmarksView;
            }
        }

        public void DeleteNewLandmark(int newLandmarkId)
        {
            using (db = new CodersEntities())
            {
                db.NewLandmarks.Remove(db.NewLandmarks.Find(newLandmarkId));
                db.SaveChanges();
            }
        }
    }
